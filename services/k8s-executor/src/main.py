"""
AutoHeal AI - K8s Executor Main Application
"""
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    k8s = K8sClient()
    app.state.k8s_client = k8s
    
    logger.info("K8s Executor initialized")
    
    yield
    
    logger.info("Shutting down K8s Executor...")


app = FastAPI(
    title="AutoHeal AI - Kubernetes Action Executor",
    description="Execute healing actions on Kubernetes clusters",
...
async def readiness_check(request: Request):
    ...
    return {"status": "ready", "k8s_connected": k8s.is_connected()}
